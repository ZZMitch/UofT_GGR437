// GEE textbook F4.6 Code

// Import Data of interest and plot it at an interesting location
// Landsat 8, scale, mask clouds
...
var l8ChartTL = ui.Chart.image.series(l8sr.select('NDVI'), roi)
...
// a 2x1 (Array Image) band called 'coefficients' (columns are for dependent variables).
var trend = l8sr.select(independents.add(dependent))
...
var detrended = l8sr.map(function(image) {
...
var harmonicLandsat = l8sr.map(function(image) {
  var timeRadians = image.select('t').multiply(2 * Math.PI);
  return image
    .addBands(timeRadians.cos().rename('cos'))
    .addBands(timeRadians.sin().rename('sin'));
});
//print(harmonicLandsat);
...
var meanNdvi= l8sr.select('NDVI').mean();
